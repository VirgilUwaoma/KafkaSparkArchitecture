    ...
    try:
        ...
        if topic_name not in metadata.topics:
            ...
            for topic, future in fs.items():
                ...
                try:
                    future.result()
                    logger.info(f"Topic '{topic_name} created successfuly")
                
                except Exception as e:
                    logger.error(f"Failed to created topic '{topic_name}': {e}")
        else: logger.info(f"Topic '{topic_name}' already exists")
    except Exception as e:
...
def delivery_report(err, msg):
    if err is not None:
        ...
        logger.info(f"Record {msg.key()} sucessfully produced")
...
def produce_data_parallel(num_threads):
    threads = []

    try:
        for i in range(num_threads):
            thread = threading.Thread(
                target=produce_transaction,
                args=(i,)
            )
            thread.daemon
            thread.start()
